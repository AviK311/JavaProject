...
public class Box extends Geometry {
    ...
    public Box(Color emission, int Shininess, double Kd, double Ks, double Kr, double Kt,
               Point3D p1, Vector v1, Vector v2, double length) {
        super(emission, Shininess, Kd, Ks, Kr, Kt);
        if (v1.dotProduct(v2) != 0)
            throw new IllegalArgumentException("Vectors must be orthogonal");
        v1 = v1.normalize();
        v2 = v2.normalize();
        Vector v3 = v1.crossProduct(v2);
        squares.add(new Square(emission, get_material(), p1, v1, v2, length));
        squares.add(new Square(emission, get_material(), p1, v1, v3, length));
        squares.add(new Square(emission, get_material(), p1, v2, v3, length));
        Point3D p2 = p1.add(v1.scale(length));
        squares.add(new Square(emission, get_material(), p2, v2, v3, length));
        p2 = p1.add(v2.scale(length));
        squares.add(new Square(emission, get_material(), p2, v1, v3, length));
        p2 = p1.add(v3.scale(length));
        squares.add(new Square(emission, get_material(), p2, v1, v2, length));
    }
